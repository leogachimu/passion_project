...
The diagram belwo illustrates the workflow I followed, from  ETL, transformation, modelling, and evaluation.
...
Three data csv files were provided for this project:
***a.) Train.csv***
...
To enable more effective analysis and modelling, I created the following columns:
i.) 'date' column by splitting the ID (eg 127_2017-01-03) to get the date string which I then converted to datetime type.
ii.) 'month' column generated from the date column.
iii.) 'year' column generated from the date column.
...
#### 4.1.1 Correlation
A plot of the correlation coefficients showed that some predictor features have significant positive correlation with the burned area, some have significant negative correlation, while about a third of them have near zero correlation. 
...
Auto ARIMA allows hyperparameter tuning of autoregressive, differencing, and moving average terms (p, q, and d), test for stationarity of the differencing term, error handling, the seasonal period, and so on. To ensure a robust and efficient process, I initialized Auto ARIMA model with the following parameter settings:
    **m=12** to set the seasonal period.
    **start_p=0**, **start_q=0** to start the parameter search from zero.
    **max_order=5** to limit the maximum combined order of the non-seasonal AR and MA terms (p, q, and d).
    **test='adf'** to use the Augmented Dickey-Fuller test to check stationarity.
    **error_action='ignore'**, **suppress_warnings=True**, and **stepwise=True** to control the tuning process and prevent unnecessary errors/warnings.
...
My best performing model had these scores:
Public Score- **0.017791797**
